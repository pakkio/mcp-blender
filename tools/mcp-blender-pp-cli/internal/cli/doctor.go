...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mcp-blender-pp-cli doctor
  mcp-blender-pp-cli doctor --json
  mcp-blender-pp-cli doctor --fail-on warn
  mcp-blender-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "mcp-blender-pp-cli", suggestion)
						} else {
