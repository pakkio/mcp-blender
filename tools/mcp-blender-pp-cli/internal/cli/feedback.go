...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MCP_BLENDER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MCP_BLENDER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mcp-blender-pp-cli/feedback")
...
When ` + "`MCP_BLENDER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MCP_BLENDER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mcp-blender-pp-cli feedback list
  mcp-blender-pp-cli feedback list --limit 5
  mcp-blender-pp-cli feedback list --json`,
