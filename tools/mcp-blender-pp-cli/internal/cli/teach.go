...
// mcp-blender-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"mcp-blender-pp-cli/internal/learn/entities"
	"mcp-blender-pp-cli/internal/learn/lookups"
	"mcp-blender-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "MCP_BLENDER_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("mcp-blender-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  mcp-blender-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  mcp-blender-pp-cli recall "<question>" --agent
  mcp-blender-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mcp-blender-pp-cli learnings list --agent
  mcp-blender-pp-cli learnings list --query "<substring>"
  mcp-blender-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  mcp-blender-pp-cli learnings forget "<question>" --resource <id>
  mcp-blender-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  mcp-blender-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  mcp-blender-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
