...
import (
	...
	"mcp-blender-pp-cli/internal/store"
)
...
type rootFlags struct {
	...
	// invocation. Mirrors the MCP_BLENDER_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mcp-blender-pp-cli",
		Short: "Manage mcp-blender resources via the mcp-blender API",
		Long: `Manage mcp-blender resources via the mcp-blender API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'mcp-blender-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mcp-blender-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'mcp-blender-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCameraLightingCmd(flags))
	rootCmd.AddCommand(newMaterialCmd(flags))
	rootCmd.AddCommand(newMeshCmd(flags))
	rootCmd.AddCommand(newPhysicsSimCmd(flags))
	rootCmd.AddCommand(newRenderPipelineCmd(flags))
	rootCmd.AddCommand(newRiggingAnimCmd(flags))
	...
	rootCmd.AddCommand(newDocsPromotedCmd(flags))
	rootCmd.AddCommand(newEvaluateSceneVisuallyPromotedCmd(flags))
	rootCmd.AddCommand(newImportOnlineAssetPromotedCmd(flags))
	rootCmd.AddCommand(newPythonExecPromotedCmd(flags))
	rootCmd.AddCommand(newSearchOnlineAssetsPromotedCmd(flags))
	rootCmd.AddCommand(newSimplifyGeometryPromotedCmd(flags))
