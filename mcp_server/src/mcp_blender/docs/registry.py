...
DOMAIN_DOCS: dict[str, dict[str, Any]] = {
    "blender_mesh": {
        ...
        "actions": {
            ...
            "simplify_geometry": "Reduce a mesh to a vertex budget while preserving its form: repairs the mesh (welds seams, drops loose geometry, closes pinhole gaps), then removes vertices from flat/dense regions first via limited dissolve and curvature-weighted decimation, protecting thin features/boundaries. Measures the result and rolls back on a failed quality gate instead of returning a broken mesh. Params: object_name, target or preset ('BACKGROUND'=10k, 'HERO'=50k, 'MAX'=100k), preserve_uv, max_deviation_pct, dry_run.",
            ...
            "separate_logical_areas": "Combine the given mesh object(s) into one working mesh, separate it into logical parts (by connectivity, materials, creases, or a vision LLM that decides the areas from a multiview render), classify+rename them via LLM into medium-level sub-assemblies and micro-level parts, and organize them under parent Empties in a macro/medium/micro hierarchy. Every separated part stays visible; the original source object(s) are renamed with a '.bak' suffix and hidden instead of deleted. Params: objects (required list of mesh object names -- multiple are combined first), lang ('it'|'en'|'hu'|'fr'|'de'|'es', default 'it'), reorg_level ('LIGHT'|'STANDARD'|'DEEP', default 'STANDARD' -- LIGHT gives fewer/coarser groups, DEEP more/finer), custom_prompt, split_method ('auto'|'loose'|'material'|'crease'|'vision', default 'auto'), sharp_angle, target_parts, min_part_faces, create_checkpoint (default true: snapshot first so runs stay comparable/restorable), split_only (default false: stop after the cut and return a pending_id + part list for per-part confirmation via confirm_separated_parts), use_vision (bool, names parts by close-up render via a vision model -- requires OPENROUTER_API_KEY), max_vision_renames (default 9999), vision_model, vision_only_generic (restrict vision pass to parts that fell back to a generic 'Part_N' name).",
        },
    },
    "blender_material": {
    ...
    "blender_scene": {
        ...
        "actions": {
            ...
            "job_delete": "Forget one finished background job. Params: job_id.",
            "job_prune": "Forget finished background jobs older than a cutoff (default 1 day). Params: older_than_days.",
            "job_submit": "Run any bridge tool as a background async job and return immediately with a job_id (poll job_status for progress/result, busy for the live HUD % bar). Params: method, params.",
