...
@pytest.mark.asyncio
async def test_blender_docs_custom_llm_recipe():
    mcp = FakeMCP()
    bridge = AsyncMock()
    register_domain_facades(mcp, bridge)
    ...
    with patch("mcp_blender.tools.domain_facades.is_configured", return_value=True), \
         patch("mcp_blender.tools.domain_facades.generate_text", new_callable=AsyncMock) as mock_gen:
        
        mock_gen.return_value = """
        {
          "title": "Custom Test Recipe",
          "description": "A test recipe generated by LLM",
          "steps": [
            {
              "step": 1,
              "tool": "blender_mesh",
              "action": "create",
              "params": {"object_type": "CUBE", "name": "CustomBox"}
            }
          ],
          "gotchas": "Ensure you are using the correct naming."
        }
        """

        result = await docs_tool(query="make a custom cube")
        
        assert "custom_llm_recipe" in result
        assert result["custom_llm_recipe"]["title"] == "Custom Test Recipe"
        assert len(result["custom_llm_recipe"]["steps"]) == 1
        assert result["custom_llm_recipe"]["steps"][0]["tool"] == "blender_mesh"
        assert result["custom_llm_recipe"]["steps"][0]["action"] == "create"
        assert result["matched_recipes"][0]["title"] == "Custom Test Recipe"
